...
for ff = 1 : n_files
    ...
    if ~isempty(mouse_dir)
        ...
        %
        mouse_stimon_times = mouse.time(mouse_initiation_flags);
        mouse_stimoff_times = mouse_stimon_times + bhv.stimulus;
        mouse_stimoff_idcs = sum(mouse_stimoff_times >= mouse.time',2);
        bhv.xposatstimoff = mouse.position_x(mouse_stimoff_idcs);
    end
    ...
    xlabel(sps(6),'Mouse x position at stimulus offset (a.u.)');
    ylabel(sps(1),'Proportion of long choices');
    ylabel(sps(6),'Proportion of long choices');
    ...
    %% plot psychometric curve (a.f.o. stimulus duration)
    ...
    %% plot psychometric curve (a.f.o. mouse position at stimulus offset)
    if ~isempty(mouse_dir)
        
        %
        xposatstimoff = discretize(bhv.xposatstimoff,10);
        xposatstimoff_set = unique(xposatstimoff);
        n_xposatstimoff = numel(xposatstimoff_set);
        
        % preallocation
        plong_avg_prev = nan(n_xposatstimoff,1);
        plong_avg = nan(n_xposatstimoff,1);
        plong_err = nan(n_xposatstimoff,1);
        
        % iterate through stimuli
        for ii = 1 : n_xposatstimoff
            stimulus_flags = xposatstimoff == xposatstimoff_set(ii);
            trial_flags = ...
                valid_flags & ...
                stimulus_flags;
            prev_flags = trial_idcs < n_trials;
        ...
        % plot psychometric performance
        plot(sps(end),xposatstimoff_set,plong_avg_prev,...
            'color',[fg_clr,.25],...
            'marker','none',...
            'linewidth',1.5);
        errorbar(sps(end),xposatstimoff_set,plong_avg,plong_err,...
            'color',[fg_clr,.25],...
            'marker','none',...
            'linewidth',.5,...
            'linestyle','none',...
            'capsize',0);
        plot(sps(end),xposatstimoff_set,plong_avg,...
            'color',fg_clr,...
            'marker','o',...
            'markeredgecolor',bg_clr,...
            'markerfacecolor',fg_clr,...
        ...
        plot(sps(end),...
            ...
            'color',fg_clr)
        plot(sps(end),...
            [1,1]*mean(xlim(sps(end))),ylim(sps(end)),':',...
            'color',fg_clr);
    end
