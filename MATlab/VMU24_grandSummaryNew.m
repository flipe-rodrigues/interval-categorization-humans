...
subject_choiceleft = cell(n_subjects,1);
subject_choicelong = cell(n_subjects,1);
subject_choicecorrect = cell(n_subjects,1);
...
for ss = 1 : n_subjects
    ...
    subject_choiceleft{ss} = choice_left(valid_flags);
    subject_choicelong{ss} = choice_long(valid_flags);
    subject_choicecorrect{ss} = choice_correct(valid_flags);
...
pool_subject = repmat(subject_name,numel(pool_stimuli),1);
pool_choiceleft = vertcat(subject_choiceleft{:});
pool_choicelong = vertcat(subject_choicelong{:});
pool_choicecorrect = vertcat(subject_choicecorrect{:});
...
bhv_table = table(...
    ...
    stimulus_table,...
    choice_table,...
    'variablenames',{'subject','stimuli','choice'});
...
%% outcome settings
pool_outcomes = pool_choicelong == (pool_stimuli > boundary);
...
for kk = 1 : n_clusters
    ...
    for ii = 1 : n_stimuli
        ...
        psy.y(ii,1) = sum(pool_choicelong(trial_flags));
        psy.n(ii,1) = sum(trial_flags);
        psy.err(ii,1) = ...
            std(pool_choicelong(trial_flags)) / sqrt(sum(trial_flags));
