...
for ss = 1 : n_subjects
    ...
    for tt = 1 : n_trials
        uscore_idcs = regexp(drawn_images{tt},'_');
        if numel(uscore_idcs) == 0
            image_id = drawn_images{tt};
        elseif numel(uscore_idcs) == 1
            image_id = drawn_images{tt}(1:uscore_idcs(1)-1);
        else
            image_id = drawn_images{tt}(1:uscore_idcs(2)-1);
        end
        type_idcs = find(cellfun(@(x)contains(image_id,x),type_set));
